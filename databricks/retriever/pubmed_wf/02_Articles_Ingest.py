...
# MAGIC
# MAGIC **Objective**: This notebook will create arguments from `pubmed_wf.raw_metadata`, and `pubmed_wf.raw_search_hist` to query PMC for new articles related to our key word search topics of interest. Upon a successful run, `pubmed_wf.raw_metadata` and `pubmed_wf.raw_search_hist` tables will be updated with search and download metadata and articles will be downloaded to `pubmed_wf.raw_articles` folder location.
# MAGIC
# MAGIC This notebook uses convenience functions imported from a <a href="$../../../python/biomed_genai/retriever/pubmed_wf/pmc.py" target="_blank">pmc</a> module. They include:
# MAGIC
...
from biomed_genai.retriever.pubmed_wf.pmc import search_articles
...
from biomed_genai.retriever.pubmed_wf.pmc import download_pending_articles
