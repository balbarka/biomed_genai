...
# MAGIC ```
# MAGIC [{'topic': topic1,'context': concatenated_chunks1},
# MAGIC  {'topic': topic2,'context': concatenated_chunks2},
# MAGIC  ...]
# MAGIC  ```

# COMMAND ----------

chunkset_df = spark.table("yen.syn_data_gen.chunksets")
display(chunkset_df)

# COMMAND ----------

def similar_chunks_as_inputs(df: pd.DataFrame) -> List[dict]:
    if df.empty:
        raise EmptyDataError("df must not be an empty pandas dataframe!")
    else:
        inputs = []
        topics = df.index.to_list()
        for t in topics:
            concatenated_chunks = "\n\n\n".join(df.content[t])
            inputs.append({'topic': t,'context': concatenated_chunks})
        return inputs

# COMMAND ----------

inputs = similar_chunks_as_inputs(chunkset_df.toPandas())
inputs
...
7. The answer to the question should be based on the given context, not contain any links or extra information.
8. Be as precise as possible with answering the question.
