...
# MAGIC
# MAGIC There isn't much that goes into Endpoint, but we still need to be aware of it's presence since scaling to many indexes will have implications of which indexes are grouped into the same endpoint (which won't be covrered in this solution accelerator).

# COMMAND ----------

biomed.vector_search.biomed.endpoint
...
biomed.vector_search.biomed.processed_articles_content_vs_index.index.sync()
...
if INSPECT_LC_INDEX:
    ...
    retriever_config = {"chunk_template": "Passage: {chunk_text}\n",
                        ...
                        "schema": {"chunk_text": "content",
                                   "document_uri": "url",
                                   "primary_key": "id"},
                        "vector_search_index": biomed.vector_search.biomed.processed_articles_content_vs_index.index.name}
    # Turn the Vector Search index into a LangChain retriever
    lc_retriever = DatabricksVectorSearch(
        index = biomed.vector_search.biomed.processed_articles_content_vs_index.index,
        text_column="content",
